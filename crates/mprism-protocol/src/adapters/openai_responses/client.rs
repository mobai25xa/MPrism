...
impl ProtocolAdapter for OpenAiResponsesAdapter {
    ...
    async fn stream_chat(
        ...
        let url = join_api_path(&endpoint.base_url, "responses")?;
        let wire = WireResponseRequest::from_request(&request);
...
impl WireResponseRequest {
    fn from_request(request: &ChatRequest) -> Self {
        // Scheme A: all messages go into `input` as pure-text role messages.
        Self {
            model: request.model.trim().to_string(),
            input: request
                .messages
                .iter()
                .map(|m| WireInputMessage {
...
mod tests {
    ...
    use crate::types::{normalize_base_url, ChatMessage, ChatRole};
    ...
    fn wire_request_stream_store_and_optionals() {
        ...
        let wire = WireResponseRequest::from_request(&req);
    ...
    fn auth_header_only_when_key_present() {
        ...
        let headers = OpenAiResponsesAdapter::auth_headers(&with_key, true);
        ...
        let headers = OpenAiResponsesAdapter::auth_headers(&no_key, false);
        assert!(!headers.contains_key(AUTHORIZATION));
    }
}
